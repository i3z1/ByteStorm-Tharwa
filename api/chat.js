...
- خطة الادخار الحالية: ${s.invest.monthly} ر.س شهرياً في حساب ادخار مرابحة (~4% سنوياً) — الهدف: ${g.amount > 0 ? `${g.name} (${g.amount} ر.س خلال ${g.months} شهراً)` : "غير محدد"}

قاعدة أساسية: الكلام وحده لا ينفّذ شيئاً — أي إجراء (بطاقة تحويل، إضافة مستفيد، خطة ادخار، ميزانية، زكاة، فتح شاشة) يتم فقط عبر استدعاء الأداة فعلياً في نفس هذا الرد. لا تقل أبداً «تم الضبط» أو «تم التنفيذ» أو إن بطاقة أو شاشة ظهرت للعميل إذا لم تستدعِ الأداة — الادعاء النصي بدون أداة يعتبر خطأً جسيماً.
...
2) execute_transfer: لا تستدعها أبداً — التنفيذ يتم حصراً بضغط العميل زر «تأكيد التحويل» في البطاقة وإدخال رمز التحقق، ولا يوجد تنفيذ عبر المحادثة. إذا كتب العميل تأكيداً نصياً («أكد»، «نفّذ»، «حوّل الآن») بعد ظهور البطاقة، لا تستدعِ أي أداة — وجّهه بلطف للضغط على زر التأكيد في البطاقة.
...
4) set_investment_plan(monthly, goal_name, goal_amount, goal_months): عند طلب خطة ادخار (أو إذا سمّاها العميل خطة استثمار) أو هدف ادخار (مثل: «أبي أجمع 30 ألف لسيارة خلال سنة» → goal_name سيارة، goal_amount 30000، goal_months 12). إذا لم يحدد العميل مبلغاً شهرياً لا ترسل monthly — سيُحسب تلقائياً القسط الذي يحقق الهدف بعائد مركّب. الادخار يتم في حساب ادخار مرابحة متوافق مع الشريعة بعائد متوقع ~4% سنوياً — لا توجد مستويات مخاطرة ولا استثمار في أسهم أو ذهب. ستُفتح شاشة الادخار تلقائياً.
...
const TOOLS = [{
  function_declarations: [
    ...
    },
    {
      ...
      description: "محجوز لمسار زر التأكيد فقط — أي استدعاء من النموذج سيُرفض دائماً لأن التنفيذ يتطلب ضغطة الزر ورمز التحقق في البطاقة. لا تستدعه أبداً.",
      parameters: {
        type: "OBJECT",
        properties: {
          amount: { type: "NUMBER", description: "المبلغ بالريال السعودي" },
    ...
    },
    {
      ...
      description: "يضبط خطة ادخار مرتبطة بهدف (يسميها بعض العملاء خطة استثمار): يحسب القسط الشهري المطلوب بعائد مرابحة مركّب (~4% سنوياً) ويفتح شاشة الادخار. مرّر الهدف إذا ذكره العميل، ولا تمرّر monthly إلا إذا حدده العميل صراحة.",
      parameters: {
        type: "OBJECT",
        properties: {
    ...
    {
      name: "open_screen",
      description: "يفتح شاشة داخل التطبيق: تحليل المصروفات (spend) أو خطة الادخار (invest) أو الرئيسية (home).",
      parameters: {
        type: "OBJECT",
        properties: {
    ...
    },
    {
      ...
      description: "يفتح تذكرة دعم لموظف البنك ويعرض بطاقة برقم مرجعي وزمن تواصل متوقع. حصراً للطلبات التي تحتاج موظفاً بشرياً: تقديم طلب تمويل بعد موافقة العميل، فتح/إغلاق حساب، شكوى معقدة. لا تستدعه لأي سؤال يمكن الإجابة عنه مباشرة، ولا لترشيح أو إصدار البطاقات (لها recommend_card و issue_card).",
      parameters: {
        type: "OBJECT",
        properties: {
...
function doExecute(args, s, actions) {
  // hard gate, absolute: transfers are NEVER executed through the model.
  // The only execution path is the deterministic confirm-button route
  // (body.confirm) with its OTP moment — a typed "نفّذ/أكد/حوّل الآن" must
  // not skip the button + verification code, whatever the model decides.
  if (!s.pending) {
    const out = doPropose(args, s, actions);
    if (!out.ok) return out;
  }
  return { ok: false, note: "التنفيذ لا يتم عبر المحادثة إطلاقاً — حماية إلزامية. بطاقة التأكيد معروضة للعميل، والتحويل يُنفَّذ حصراً بضغطة زر «تأكيد التحويل» في البطاقة وإدخال رمز التحقق. اطلب من العميل بلطف الضغط على الزر في البطاقة، ولا تقل أبداً إن التحويل تم." };
}
...
function doInvest(args, s, actions) {
  ...
  if (!(monthly > 0)) monthly = neededMonthly(g.amount, SAVE_RATE, g.months);
  monthly = Math.max(250, Math.min(20000, Math.round(monthly / 50) * 50));
  const projected = fvMonthly(monthly, SAVE_RATE, g.months);
  const surplus = Math.max(0, Math.round(s.income - Object.values(s.expenses).reduce((a, b) => a + b, 0)));
  s.invest = { monthly, goal: g };
  actions.push({ type: "invest_plan", monthly, goal: g, projected });
  ...
  return { ok: true, note: `الخطة: ${monthly} ر.س شهرياً في حساب ادخار مرابحة (~${SAVE_RATE}% سنوياً). الهدف: ${g.name} — ${g.amount} ر.س خلال ${g.months} شهراً. المتوقع تجميعه بعائد مركّب: ~${projected} ر.س (${fit}).${afford} فُتحت شاشة الادخار للعميل.` };
}
...
export default async function handler(req, res) {
  ...
  function cannedReply() {
    ...
    if (last.type === "invest_plan") return `جهّزت لك الخطة: ${last.monthly} ر.س شهرياً في حساب ادخار مرابحة — افتح شاشة الادخار وشوف التفاصيل.`;
